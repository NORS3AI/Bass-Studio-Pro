...
const Player = (() => {
  ...
  function handleVisibilityChange() {
    if (document.hidden) {
      ...
      // Debounce: iOS/WebKit fires spurious visibility events (url bar, etc).
      // Only swap to backup if we're still hidden after 250ms.
      if (bgSwapTimer) clearTimeout(bgSwapTimer);
      bgSwapTimer = setTimeout(() => {
        bgSwapTimer = null;
        if (document.hidden && isPlaying && currentTrack && currentTrack.url && !backupAudio) {
          startBackupAudio();
        }
      }, 250);
    } else {
  ...
  function stopBackupAudio() {
    ...
    if (backupWasPlaying) {
      isPlaying = true;
      emit('statechange', 'playing');
      // Play FIRST, then seek after playback has actually resumed. On iOS/WebKit,
      // setting currentTime before calling play() on a MediaElementSource-connected
      // element can break the audio routing to the AudioContext destination.
      const p = audioElement.play();
      const doSeek = () => { try { audioElement.currentTime = backupTime; } catch (_) {} };
      ...
      try { audioElement.currentTime = backupTime; } catch (_) {}
    }
  }
